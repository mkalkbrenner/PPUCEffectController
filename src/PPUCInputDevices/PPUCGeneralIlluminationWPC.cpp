...
void PPUCGeneralIlluminationWPC::start() {
    ...
    attachInterrupt(digitalPinToInterrupt(2), PPUCGeneralIlluminationWPC::_changeD0, RISING);
    attachInterrupt(digitalPinToInterrupt(3), PPUCGeneralIlluminationWPC::_changeD1, RISING);
    attachInterrupt(digitalPinToInterrupt(4), PPUCGeneralIlluminationWPC::_changeD2, RISING);
    attachInterrupt(digitalPinToInterrupt(5), PPUCGeneralIlluminationWPC::_changeD3, RISING);
    attachInterrupt(digitalPinToInterrupt(6), PPUCGeneralIlluminationWPC::_changeD4, RISING);
    attachInterrupt(digitalPinToInterrupt(12), PPUCGeneralIlluminationWPC::_changeZC, FALLING);
}
...
void PPUCGeneralIlluminationWPC::update() {
    for (int string = 0; string < NUM_GI_STRINGS; string++) {
        if (sBrightness[string] != sBrightnessTarget[string]) {
            ...
            eventDispatcher->dispatch(new PPUCEvent(EVENT_SOURCE_GI, word(0, string + 1), sBrightness[string]));
        }
    }
}
