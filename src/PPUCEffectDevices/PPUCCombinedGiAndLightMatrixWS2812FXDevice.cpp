...
void PPUCCombinedGiAndLightMatrixWS2812FXDevice::assignLedToGiString(uint8_t giString, int8_t led, uint32_t color) {
    if (numLEDsGI[--giString] < PPUC_MAX_LEDS_GI_STRING) {
...
void PPUCCombinedGiAndLightMatrixWS2812FXDevice::updateAfterGlow() {
    ...
    for (uint8_t number = 0; number < PPUC_LIGHT_MATRIX_SIZE; number++) {
...
void PPUCCombinedGiAndLightMatrixWS2812FXDevice::setDimmedPixelColor(int led, uint32_t color, uint8_t brightness) {
